...
public class MazeSolver {
    ...
    public static class BFSInstance
    {
        private CustomQueue<Point> queue = new CustomQueue<>();
        private boolean[][] visited;
        private int[][] maze;
        private int[] end;
        public boolean solved = false;
        private Point finalPoint = null; // Store final point to reconstruct path

        public boolean impossible = false;

        public BFSInstance(int[][] maze, int[] start, int[] end)
        {
            this.maze = maze;
            this.end = end;
        ...
        public Point step() {
            ...
            // Only proceed if we aren't already finished or in a failure state
            if (!solved && !impossible) {

                // 1. Check for impossible state (Empty queue means no path)
                if (queue.isEmpty()) {
                    impossible = true;
                }
                else {
                    ...
                    if (current.x == end[0] && current.y == end[1]) {
                        ...
                        for (int[] d : directions) {
                            ...
                            if (nx >= 0 && nx < maze.length && ny >= 0 && ny < maze[0].length
                                    && (maze[nx][ny] == 0 || maze[nx][ny] == 9) && !visited[nx][ny]) {
                                visited[nx][ny] = true;
                                queue.enQueue(new Point(nx, ny, current));
                            }
                        }
                        result = current; // Return the current step
                    }
                }
            }
        ...
        // Add this method to extract solved path once solved == true
        public List<int[]> getPath() {
            List<int[]> path = new ArrayList<>();
            Point temp = finalPoint;
            while (temp != null) {
    ...
    public static class DFSInstance {
        private CustomStack<Point> stack = new CustomStack<>();
        private boolean[][] visited;
        private int[][] maze;
        private int[] end;
        public boolean solved = false;
        ...
        public DFSInstance(int[][] maze, int[] start, int[] end) {
            this.maze = maze;
            this.end = end;
            this.visited = new boolean[maze.length][maze[0].length];
        ...
        public Point step() {
            ...
            if (!solved && !impossible) {
                ...
                // 1. Check for impossible state (Empty stack means no path)
                if (stack.isEmpty()) {
                    impossible = true;
                }
                else {
                    // 2. Main DFS logic
                    Point current = stack.popItem();
